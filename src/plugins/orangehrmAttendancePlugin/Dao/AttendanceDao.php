...
class AttendanceDao extends BaseDao
{
    ...
    public function getAttendanceRecord(int $employeeNumber, string $date): ?AttendanceRecord
    {
        // Create a query to fetch the attendance record based on employee number and date
        $q = $this->createQueryBuilder(AttendanceRecord::class, 'attendanceRecord');
        $q->andWhere('attendanceRecord.employee = :empNumber');
        $q->andWhere('attendanceRecord.date = :date')
    ...
    /**
     * Search for attendance records based on criteria.
     *
     * @param array $criteria
     * @return AttendanceRecord[]
     */
    public function searchAttendanceRecords(array $criteria): array
    {
        // Start by creating a QueryBuilder for the AttendanceRecord entity
        $qb = $this->createQueryBuilder(AttendanceRecord::class, 'attendanceRecord');
        
        // Add criteria for employee number if it exists in the array
        if (isset($criteria['employeeNumber'])) {
            $qb->andWhere('attendanceRecord.employee = :employeeNumber')
               ->setParameter('employeeNumber', $criteria['employeeNumber']);
        }
        
        // Add criteria for date if it exists in the array
        if (isset($criteria['date'])) {
            $qb->andWhere('attendanceRecord.date = :date')
               ->setParameter('date', $criteria['date']);
        }

        // Additional criteria can be added here in a similar fashion
